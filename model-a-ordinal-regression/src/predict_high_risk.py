...
用法:
  python predict_high_risk.py 数据.xlsx
  python predict_high_risk.py 数据.csv
  或在代码中: from predict_high_risk import predict; result = predict(df)
"""
...
MODEL_PATH = os.path.join(SCRIPT_DIR, "saved_models", "high_risk_model.pkl")
...
def predict(df_raw, model_path=None, return_proba=False):
    ...
    返回:
        如果 return_proba=False: pandas Series，索引为输入DataFrame的索引，值为预测等级标签
    ...
    if return_proba:
        proba_df = pd.DataFrame(
            proba, columns=[f"P(等级{i})" for i in range(K)],
            index=df_raw.index
        )
...
def main():
    if len(sys.argv) < 2:
        print("用法: python predict_high_risk.py <数据文件.xlsx或.csv>")
        print("示例: python predict_high_risk.py 新患者数据.xlsx")
        sys.exit(1)
    ...
    print(f"\n{'='*50}")
    print(f"  高风险预警模型 - 预测结果 (共 {len(result_df)} 条)")
    print(f"{'='*50}")
    ...
    # 高风险患者单独列出
    high_risk = result_df[result_df["风险等级"] == "高风险"]
    if len(high_risk) > 0:
        print(f"\n  ★ 预测为高风险的患者 ({len(high_risk)} 人):")
        print(high_risk.to_string(max_rows=50))
    ...
    out_path = input_path.replace(suffix, "_高风险预警结果.csv")
